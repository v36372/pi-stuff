...
// ==========================================================================
// OAuth compatibility (pi 0.84+ emptied @earendil-works/pi-ai/oauth)
// ==========================================================================

type CopilotCredentials = OAuthCredentials & { enterpriseUrl?: string };
type GeminiCredentials = OAuthCredentials & { projectId?: string };

/** Minimal legacy OAuth shape still accepted by pi.registerProvider(). */
interface LegacyOAuthConfig {
	name: string;
...
/** New pi-ai OAuthAuth surface used by built-in providers. */
interface ModernOAuthAuth {
	name: string;
	isSubscription?: boolean;
	login(interaction: {
		signal: AbortSignal;
	...
	refresh(credential: OAuthCredentials & { type: "oauth" }, signal: AbortSignal): Promise<OAuthCredentials & { type: "oauth" }>;
	toAuth(credential: OAuthCredentials & { type: "oauth" }): Promise<{ apiKey?: string; baseUrl?: string }>;
}

function resolvePiAiRoot(): string {
	const specs = [
	...
	for (const spec of specs) {
		try {
			...
			if (existsSync(join(dir, "dist/auth/oauth/openai-codex.js"))) {
				return dir;
			}
		} catch {
			// try next specifier
	...
	const codingAgentRoots: string[] = [];
	if (typeof process.argv[1] === "string") {
		codingAgentRoots.push(dirname(dirname(process.argv[1])));
	}
	for (const root of codingAgentRoots) {
		for (const name of ["@earendil-works/pi-ai", "@mariozechner/pi-ai"]) {
			const candidate = join(root, "node_modules", name);
			if (existsSync(join(candidate, "dist/auth/oauth/openai-codex.js"))) {
				return candidate;
			}
		}
	}
	...
	throw new Error(
		"Could not locate pi-ai OAuth flows. Reinstall @earendil-works/pi-coding-agent or update multipass.",
	);
}

const oauthModuleCache = new Map<string, Promise<ModernOAuthAuth>>();

async function loadModernOAuth(moduleName: "anthropic" | "openai-codex" | "github-copilot"): Promise<ModernOAuthAuth> {
	const cached = oauthModuleCache.get(moduleName);
	if (cached) return cached;

	const pending = (async () => {
		const root = resolvePiAiRoot();
		const mod = await import(pathToFileURL(join(root, `dist/auth/oauth/${moduleName}.js`)).href);
		const exportName =
			moduleName === "anthropic"
				? "anthropicOAuth"
				: moduleName === "openai-codex"
					? "openaiCodexOAuth"
					: "githubCopilotOAuth";
		const oauth = mod[exportName] as ModernOAuthAuth | undefined;
		if (!oauth?.login || !oauth.refresh) {
			throw new Error(`pi-ai OAuth module "${moduleName}" is missing ${exportName}`);
		}
		return oauth;
	})();

	oauthModuleCache.set(moduleName, pending);
	return pending;
}
...
function wrapModernOAuth(
	oauthLoader: () => Promise<ModernOAuthAuth>,
	name: string,
	...
	return {
		...
		async login(callbacks) {
			const oauth = await oauthLoader();
			const credential = await oauth.login(toAuthInteraction(callbacks));
			const { type: _type, ...rest } = credential as OAuthCredentials & { type?: string };
			return rest;
		},
		async refreshToken(credentials, signal) {
			...
			const { type: _type, ...rest } = refreshed;
			return rest;
		},
...
function unsupportedGoogleOAuth(name: string): LegacyOAuthConfig {
	const message =
		`${name} OAuth was removed from pi 0.84's public oauth API. Extra Google accounts cannot be logged in with this multipass build.`;
	return {
		name,
		isSubscription: true,
...
function getCopilotBaseUrl(token?: string, enterpriseDomain?: string): string {
	if (token) {
		const match = token.match(/proxy-ep=([^;]+)/);
		if (match) return `https://${match[1].replace(/^proxy\./, "api.")}`;
	}
	if (enterpriseDomain) {
		const host = enterpriseDomain.replace(/^https?:\/\//, "").split("/")[0];
...
async function refreshGoogleCloudToken(_refresh: string, _projectId: string): Promise<GeminiCredentials> {
	throw new Error(
		"Google Cloud Code Assist token refresh is unavailable on pi 0.84; re-login is not supported for extra Google accounts.",
	);
}

async function refreshAntigravityToken(_refresh: string, _projectId: string): Promise<GeminiCredentials> {
	throw new Error(
		"Antigravity token refresh is unavailable on pi 0.84; re-login is not supported for extra Google accounts.",
	);
}
...
interface ProviderTemplate {
	displayName: string;
	usesCallbackServer?: boolean;
	buildOAuth(index: number): LegacyOAuthConfig;
	buildModifyModels?(providerName: string): LegacyOAuthConfig["modifyModels"];
}
...
const PROVIDER_TEMPLATES: Record<string, ProviderTemplate> = {
	anthropic: {
		...
		buildOAuth(index: number) {
			return wrapModernOAuth(() => loadModernOAuth("anthropic"), `Anthropic #${index}`);
		},
	},
	...
	"openai-codex": {
		...
		usesCallbackServer: true,
		buildOAuth(index: number) {
			return wrapModernOAuth(() => loadModernOAuth("openai-codex"), `ChatGPT Codex #${index}`, {
				usesCallbackServer: true,
			});
		},
	},
	...
	"github-copilot": {
		...
		buildOAuth(index: number) {
			return wrapModernOAuth(() => loadModernOAuth("github-copilot"), `GitHub Copilot #${index}`);
		},
		buildModifyModels(providerName: string) {
			return (models: Model<Api>[], credentials: OAuthCredentials): Model<Api>[] => {
				...
				const domain = creds.enterpriseUrl
					...
					: undefined;
				const baseUrl = getCopilotBaseUrl(creds.access, domain);
				return models.map((m) =>
	...
	"google-gemini-cli": {
		...
		usesCallbackServer: true,
		buildOAuth(index: number) {
			return unsupportedGoogleOAuth(`Google Cloud Code Assist #${index}`);
		},
	},
	...
	"google-antigravity": {
		...
		usesCallbackServer: true,
		buildOAuth(index: number) {
			return unsupportedGoogleOAuth(`Antigravity #${index}`);
		},
	},
};
...
/** Compatibility probe for auth checks after ModelRegistry.authStorage was removed (pi >= 0.80.8). */
type AuthProbe = {
...
type ModelRegistryAuth = {
	getProviderAuthStatus(provider: string): { configured: boolean };
};

function authProbe(modelRegistry: ModelRegistryAuth): AuthProbe {
	return {
		hasAuth(provider: string): boolean {
		...
		logout(provider: string): void {
			logoutStoredCredential(provider);
		},
	};
}

function logoutStoredCredential(provider: string): void {
	...
	try {
		...
		// Best-effort; callers still refresh the model registry afterward.
	}
}
...
function collectQuotaAccounts(ctx: ExtensionContext): QuotaAccount[] {
	...
	const pushAccount = (providerName: string, displayName: string) => {
		...
		accounts.push({
			...
			displayName,
			auth: authProbe(ctx.modelRegistry).get(providerName) as AuthStorageEntry | undefined,
		});
	};

	for (const checker of PROVIDER_QUOTA_CHECKERS) {
		if (authProbe(ctx.modelRegistry).hasAuth(checker.baseProvider)) {
			pushAccount(
...
function getProjectScopedProviderNames(
	...
	for (const providerName of SUPPORTED_PROVIDERS) {
		if (authProbe(ctx.modelRegistry).hasAuth(providerName)) {
			push(providerName);
		}
	}
...
function getSwitchableProviderOptions(
	...
	const push = (providerName: string, label: string, description: string) => {
		if (allowed && !allowed.has(providerName)) return;
		if (!authProbe(ctx.modelRegistry).hasAuth(providerName)) return;
		if (seen.has(providerName)) return;
		seen.add(providerName);
...
async function removeSubscriptionEntry(
	...
	const name = subProviderName(entry);
	if (authProbe(ctx.modelRegistry).hasAuth(name)) {
		authProbe(ctx.modelRegistry).logout(name);
	}
...
async function showSubscriptionActions(
			...
			items: [
				{
					...
					label: formatSubscriptionListLine(entry, config, authProbe(ctx.modelRegistry)),
				},
			],
			confirmHint: "back",
			cancelHint: "back",
	...
	const name = subProviderName(entry);
	const hasAuth = authProbe(ctx.modelRegistry).hasAuth(name);
	...
	while (true) {
		...
		const selectedProviderName = await showWrappedSelect(ctx, {
			...
			items: all.map((entry) => ({
				...
				description: formatSubscriptionMeta(entry, config, authProbe(ctx.modelRegistry)),
			})),
...
async function handleSubsRemove(
	...
	const selectedProviderName = await showWrappedSelect(ctx, {
		...
		items: config.subscriptions.map((entry) => ({
			value: subProviderName(entry),
			label: subDisplayName(entry),
			description: authProbe(ctx.modelRegistry).hasAuth(subProviderName(entry))
				? "logged in"
				: "not logged in",
		})),
		confirmHint: "remove",
...
async function handleSubsLogin(ctx: ExtensionCommandContext): Promise<void> {
	...
	const notLoggedIn = all.filter(
		(entry) => !authProbe(ctx.modelRegistry).hasAuth(subProviderName(entry)),
	);
...
async function handleSubsLogout(ctx: ExtensionCommandContext): Promise<void> {
	...
	const loggedIn = all.filter((entry) =>
		authProbe(ctx.modelRegistry).hasAuth(subProviderName(entry)),
	);
	...
	authProbe(ctx.modelRegistry).logout(subProviderName(entry));
	await ctx.modelRegistry.refresh();
	ctx.ui.notify(`Logged out of ${subDisplayName(entry)}`, "info");
}
...
async function handleSubsStatus(ctx: ExtensionCommandContext): Promise<void> {
	...
	for (const entry of all) {
		const name = subProviderName(entry);
		const cred = authProbe(ctx.modelRegistry).get(name);
		const hasAuth = authProbe(ctx.modelRegistry).hasAuth(name);
...
async function editPoolMembers(
	...
	while (true) {
		const removableItems: SelectItem[] = selectedMembers.map((member) => ({
			value: `remove:${member}`,
			label: `remove ${member}`,
			description: authProbe(ctx.modelRegistry).hasAuth(member) ? "logged in" : "not logged in",
		}));
		const addableItems: SelectItem[] = availableProviders
			...
			.map((providerName) => ({
				...
				description: authProbe(ctx.modelRegistry).hasAuth(providerName)
					? "logged in"
					: "not logged in",
			}));
...
async function promptForPoolDefinition(
	...
	const authedProviders = allProviders.filter((p) =>
		authProbe(ctx.modelRegistry).hasAuth(p),
	);
	...
	while (selecting) {
		...
		const optionsList = [
			...
			...remaining.map((p) => {
				const authed = authProbe(ctx.modelRegistry).hasAuth(p);
...
async function inspectPoolConfig(
	...
	await showWrappedSelect(ctx, {
		...
		items: formatPoolStatusLines(pool, authProbe(ctx.modelRegistry), poolManager)
	...
	while (true) {
		...
		const selectedPoolName = await showWrappedSelect(ctx, {
			...
			items: pools.map((pool) => ({
				value: pool.name,
				label: pool.name,
				description: formatPoolListDescription(pool, authProbe(ctx.modelRegistry), poolManager),
			})),
...
async function handlePoolStatus(
	...
	for (const pool of config.pools) {
		lines.push(
			...formatPoolStatusLines(pool, authProbe(ctx.modelRegistry), poolManager),
		);
	}
...
async function handlePoolChainList(
	...
	await ctx.ui.select(
		...
		config.chains.map((chain) =>
			formatChainListLine(chain, config, authProbe(ctx.modelRegistry), poolManager),
		),
	);
}
...
async function handlePoolChainStatus(
	...
	await ctx.ui.select(
		...
		formatChainStatusLines(chain, config, authProbe(ctx.modelRegistry), poolManager),
	);
}
...
async function handlePoolProject(
	...
	if (action === "info") {
		...
		for (const sub of effective.subscriptions) {
			const authed = authProbe(ctx.modelRegistry).hasAuth(subProviderName(sub));
...
async function handlePresetActivate(
	...
	for (const entry of preset.entries) {
		...
		if (!authProbe(ctx.modelRegistry).hasAuth(entry.provider)) continue;
