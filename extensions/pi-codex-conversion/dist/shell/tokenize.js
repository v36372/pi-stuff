...
function canEmitUnquoted(token) {
...
function isUnquotedOk(char) {
    return /^[+\-./:@\]_0-9A-Za-z]$/.test(char);
}
export function joinCommandTokens(tokens) {
    return tokens
...
export function splitOnConnectors(tokens) {
    const parts = [];
    let current = [];
...
export function shortDisplayPath(path) {
...
export function joinPaths(base, extra) {
