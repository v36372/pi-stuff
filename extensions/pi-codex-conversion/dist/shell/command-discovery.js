import { firstNonFlagOperand, parseFdQueryAndPath, parseFindQueryAndPath, skipFlagValues, trimAtConnector, } from "./command-operands.js";
import { listCommand, searchCommand, unknownCommand, } from "./parsed-command.js";
import { joinCommandTokens } from "./tokenize.js";
export function summarizeDiscoveryCommand(head, tail, mainCommand) {
    ...
    if (head === "tree") {
        return listCommand(mainCommand, firstNonFlagOperand(tail, [
            "-L",
            "-P",
            "-I",
            "--charset",
    ...
    if (head === "rg" || head === "rga" || head === "ripgrep-all") {
        ...
        if (hasFilesFlag)
            return listCommand(mainCommand, nonFlags[0]);
    ...
    if (head === "git") {
        ...
        if (subcommand === "ls-files") {
            return listCommand(mainCommand, firstNonFlagOperand(subtail, [
                "--exclude",
                "--exclude-from",
                "--pathspec-from-file",
            ]));
