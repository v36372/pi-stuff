...
function createExecTool(runtime, tracker, renderResult) {
    return {
        name: "exec",
        label: "Exec",
        ...
        async execute(id, params, signal, onUpdate, ctx) {
            tracker.start(id);
            try {
                const response = await (await runtime.getClient()).execute(params.code, { cwd: ctx.cwd, extensionContext: ctx, onUpdate }, signal, runtime.collectTools(ctx));
...
function createWaitTool(runtime, tracker, renderResult, waitAttempts) {
    return {
        name: "wait",
        label: "Wait",
...
async function continueExecSessionFromMistakenWait(cellId, yieldTimeMs, maxOutputTokens, runtime, context, signal) {
    ...
    try {
        ...
        if (/unknown process id/i.test(fallbackMessage))
            return undefined;
        throw fallbackError;
    }
...
function createResultRenderer(runtime, tracker) {
    return (result, options, theme, context) => renderTrackedCodeModeResult(result, options, theme, context, tracker, runtime.collectRenderTools(), runtime.useRichRendering());
}
