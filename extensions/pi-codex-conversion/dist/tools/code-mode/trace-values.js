...
export function toolResultFromValue(value) {
    return {
        content: [
            {
                type: "text",
...
export function boundRuntimeToolResult(result, imageCharsRemaining) {
    ...
    for (const item of result.content) {
        ...
        content.push(sanitizeValue(item, {
...
export function truncateTraceText(text, remaining) {
    if (remaining <= 0)
        return "";
...
export function sanitizeTraceInput(value, maxChars) {
...
function sanitizeValue(value, budget) {
    ...
    if (typeof value !== "object")
        return String(value);
    const seen = budget.seen ?? new WeakSet();
    if (seen.has(value))
        return "[circular]";
    seen.add(value);
