...
export function toCodeModeToolResult(response, maxTokens) {
    ...
    output.unshift(...runningExecSessionGuidance(response.traces ?? []).map((text) => ({
        type: "text",
        text,
    })));
    if (omittedImages > 0)
...
function runningExecSessionGuidance(traces) {
