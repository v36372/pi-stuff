...
import { DEFAULT_CODEX_CONVERSION_CONFIG, REALTIME_V3_VOICES, V2_USER_MESSAGE_RETENTION_OPTIONS, WEB_SEARCH_MODELS, normalizeCodexVerbosity, normalizeProviderList, normalizeRealtimeV3Voice, normalizeV2UserMessageRetention, normalizeWebSearchModel, } from "../../adapter/activation/config.js";
...
function setting(item, update) {
    return { item, ...(update ? { update } : {}) };
}
function toggle(id, label, current, update) {
...
export function buildConfigSettings(tab, config, theme) {
    if (tab === "adapter") {
        return [
            setting({ id: "extensionMode", label: "Extension mode", currentValue: config.voiceFeaturesOnly ? "voice only" : "adapter and voice", values: ["adapter and voice", "voice only"] }, (value, current) => ({ ...current, voiceFeaturesOnly: value === "voice only" })),
            setting({ id: "allProviders", label: "Provider scope", currentValue: formatAllProvidersMode(config.scope.allProviders), values: ["Codex and configured", "all providers", "extra tools only"] }, (value, current) => ({ ...current, scope: { ...current.scope, allProviders: parseAllProvidersMode(value) } })),
            setting({
            ...
            setting({ id: "heavySystemPromptOverwrite", label: "Heavy system prompt overwrite", currentValue: config.prompt.heavySystemPromptOverwrite ? "on (40% smaller)" : "off", values: ["off", "on (40% smaller)"] }, (value, current) => ({ ...current, prompt: { ...current.prompt, heavySystemPromptOverwrite: value !== "off" } })),
    ...
    if (tab === "openai") {
        return [
            ...
            setting({ id: "harnessIdentifierHeader", label: "Harness identifier header", currentValue: config.openai.harnessIdentifierHeader ? "pi-codex-conversion <3" : "off", values: ["off", "pi-codex-conversion <3"] }, (value, current) => ({ ...current, openai: { ...current.openai, harnessIdentifierHeader: value !== "off" } })),
