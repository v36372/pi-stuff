...
import { createImageGenerationTool } from "../tools/imagegen/tool.js";
import { createViewImageTool } from "../tools/view-image/tool.js";
import { createWebSearchTool } from "../tools/web-run/tool.js";
import { supportsNativeImageGeneration, supportsViewImageInputs } from "./tool-support.js";
...
export async function registerCodexCodeMode(pi, runtime) {
    const isActive = (ctx) => resolveCodexRuntimePlan(ctx, runtime.state.config).kind === "code";
    const customToolsRuntime = await registerCustomTools(pi, undefined, {
        isActive,
    });
    const programmaticRuntime = await registerCodeModeTools(pi, {
        getTools: (ctx) => createNestedTools(runtime, ctx),
        isActive,
...
function createNestedTools(runtime, ctx) {
    const options = {
    ...
    const allowConfiguredProvider = (model) => (model?.provider ?? "").trim().toLowerCase() !== "openai-codex"
    ...
    const tools = [
        ...
        toNestedTool(createExecCommandTool(runtime.tracker, runtime.sessions, options), "await tools.exec_command({ cmd: string, workdir?: string, shell?: string, tty?: boolean, yield_time_ms?: number, max_output_tokens?: number, login?: boolean })", {
            start(id, input) {
                const cmd = input &&
                    typeof input === "object" &&
                    "cmd" in input &&
        ...
        toNestedTool(createWriteStdinTool(runtime.sessions, options), "await tools.write_stdin({ session_id: number, chars?: string, yield_time_ms?: number, max_output_tokens?: number })", {}, { yieldTimeMs: LONG_RUNNING_TOOL_OUTER_YIELD_MS }),
    ];
    ...
    if (runtime.state.config.tools.webRun) {
        tools.push(toNestedTool(createWebSearchTool("web__run", {
            customRustBinariesDir: runtime.state.config.tools.customRustBinariesDir,
    ...
    if (runtime.state.config.tools.imageGeneration && (!ctx || supportsNativeImageGeneration(ctx.model) || allowConfiguredProvider(ctx.model))) {
        ...
        tools.push(toNestedTool({ ...imagegen, name: "image_gen__imagegen", label: "image_gen__imagegen" }, "await tools.image_gen__imagegen({ prompt: string, action?: \"generate\" | \"edit\", images?: string[] })", {}, {
            resultValue(result) {
...
function isRunningExecResult(details) {
