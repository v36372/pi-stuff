 ...
 *
 * Sources / observed Cursor caps (2026-07):
 *  - Claude: default 200k; newer Opus 4.7/4.8, Sonnet 5, Fable 5 advertise 300k/1m
 *    in the Cursor SDK catalog — use 300k as the non-Max default for those.
 *  - Gemini: ai.google.dev/gemini-api/docs — all 2.5 / 3.x models: 1M.
 *  - GPT: Cursor SDK catalog uses 272k non-Max for 5.x; GPT-5.5+ also expose 1M.
 *    Register 272k for 5.x and 1M for 5.5+ / 5.6 so pi compaction tracks Cursor.
 *  - Grok 4 / 4.5: 256k (SDK often reports 200k; prefer 256k for Grok-branded IDs).
...
export function inferContextWindow(id: string): number {
  ...
  // Any model with an explicit -1m / @1m marker
  if (lower.includes("-1m") || lower.includes("@1m")) return 1_048_576;
  if (lower.includes("@300k")) return 300_000;
  if (lower.includes("@272k")) return 272_000;

  // ── Claude ────────────────────────────────────────────────────────────────
  // Newer family-first IDs: Opus 4.7/4.8, Sonnet 5, Fable 5 → 300k default.
  if (
    /claude-(opus-4-[78]|sonnet-5|fable-5)/.test(lower) ||
    /claude-4\.[78]-opus/.test(lower)
  ) {
    return 300_000;
  }
  // All other Claude (4.6 and earlier, Haiku, …): 200k.
  ...
  // ── GPT ───────────────────────────────────────────────────────────────────
  // nano / mini variants: 128k when not otherwise specified by Cursor cache.
  // GPT-5.5 / 5.6: 1M. Other GPT-5.x: 272k (Cursor non-Max default).
  if (/^gpt-[0-9.]*-(nano|mini)/.test(lower) || lower === "gpt-5-mini") {
    return 128_000;
  }
  if (/^gpt-5\.(5|6)/.test(lower)) return 1_048_576;
  if (lower.startsWith("gpt-5")) return 272_000;
  if (lower.startsWith("gpt-")) return 400_000;

  // ── Grok ──────────────────────────────────────────────────────────────────
  if (lower.startsWith("grok-") || lower.startsWith("cursor-grok-")) {
    return 256_000;
  }
...
/**
 * Insert reasoning effort into model ID.
 *
 * Legacy (GPT / older Claude):
 *   gpt-5.4 + medium → gpt-5.4-medium
 *   gpt-5.4-fast + high → gpt-5.4-high-fast
 *   claude-4.6-opus-thinking + high → claude-4.6-opus-high-thinking
 *
 * New Claude family-first IDs place effort after -thinking:
 *   claude-opus-4-8-thinking + high → claude-opus-4-8-thinking-high
 *   claude-opus-4-8-thinking-fast + high → claude-opus-4-8-thinking-high-fast
 *
...
export function resolveModelId(
  ...
  const thinkingThenEffort =
    /^claude-(opus|sonnet|haiku|fable)-/i.test(base);

  let id = base;
  if (thinkingThenEffort) {
