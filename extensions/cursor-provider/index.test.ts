...
describe("parseModelId", () => {
  ...
  test("extra-high multi-segment effort", () => {
  ...
  test("new Claude order: thinking then effort", () => {
    expect(parseModelId("claude-opus-4-8-thinking-high")).toEqual({
  ...
  test("new Claude order: thinking + effort + fast", () => {
    expect(parseModelId("claude-opus-4-8-thinking-high-fast")).toEqual({
      base: "claude-opus-4-8",
      effort: "high",
      fast: true,
      thinking: true,
  ...
  test("new Claude non-thinking effort", () => {
    expect(parseModelId("claude-sonnet-5-max")).toEqual({
  ...
  test("gemini minimal effort", () => {
    expect(parseModelId("gemini-3.6-flash-minimal")).toEqual({
  ...
  test("cursor-grok effort + fast", () => {
    expect(parseModelId("cursor-grok-4.5-high-fast")).toEqual({
...
describe("reasoning support", () => {
  ...
  test("fallback models keep derived reasoning enabled", () => {
    ...
    expect(
      FALLBACK_MODELS.find((model) => model.id === "claude-opus-4-8-thinking-high")
        ?.reasoning,
    ).toBe(true);
  });
});
...
describe("processModels", () => {
  ...
  test("claude-opus-4-8 — high+max deduped, effort clamped to lowest", () => {
    const result = processModels([
      m("claude-opus-4-8-high"),
      m("claude-opus-4-8-max"),
    ]);
    expect(result).toHaveLength(1);
  ...
  test("legacy version-first Claude IDs are dropped", () => {
    const result = processModels([
      m("claude-4.6-opus-high"),
      m("claude-4.6-opus-max"),
      m("claude-4.5-opus-high"),
      m("claude-4.6-sonnet-medium"),
      m("composer-2.5"),
    ]);
    expect(result.map((r) => r.id)).toEqual(["composer-2.5"]);
  });

  test("claude-opus-4-5-high — single effort variant, deduped to base", () => {
    const result = processModels([m("claude-opus-4-5-high")]);
    expect(result).toHaveLength(1);
    expect(result[0]!.id).toBe("claude-opus-4-5");
    expect(result[0]!.supportsEffort).toBe(true);
    expect(result[0]!.effortMap!.high).toBe("high");
    expect(result[0]!.effortMap!.minimal).toBe("high");
  });

  test("claude-sonnet-5-medium — single effort variant, deduped to base", () => {
    const result = processModels([m("claude-sonnet-5-medium")]);
    expect(result).toHaveLength(1);
  ...
  test("strips effort words from display name", () => {
    const result = processModels([
  ...
  test("cursor-grok-4.5-fast does not keep Medium in display name", () => {
    const result = processModels([
    ...
    expect(result).toHaveLength(1);
    expect(result[0]!.id).toBe("cursor-grok-4.5-fast");
    expect(result[0]!.name).toBe("Cursor Grok 4.5 Fast");
    expect(result[0]!.effortMap!.high).toBe("high");
  });
  ...
  test("full raw model list dedup count", () => {
    ...
    // Spot checks for the current Cursor catalog
    ...
    const gpt56 = result.find((r) => r.id === "gpt-5.6-sol");
    expect(gpt56).toBeDefined();
    expect(gpt56!.supportsEffort).toBe(true);

    // New family-first Opus IDs should be deduped
    const opus48 = result.find((r) => r.id === "claude-opus-4-8");
    expect(opus48).toBeDefined();
    expect(opus48!.supportsEffort).toBe(true);
    expect(result.find((r) => r.id === "claude-opus-4-8-high")).toBeUndefined();
    expect(result.find((r) => r.id === "claude-opus-4-8-max")).toBeUndefined();

    const opus48Thinking = result.find((r) => r.id === "claude-opus-4-8-thinking");
    expect(opus48Thinking).toBeDefined();
    expect(opus48Thinking!.supportsEffort).toBe(true);
    ...
    expect(
      result.find((r) => r.id === "claude-opus-4-8-thinking-high"),
    ).toBeUndefined();
  });

  test("new Claude thinking-then-effort variants dedupe", () => {
    const result = processModels([
      m("claude-opus-4-8-thinking-high"),
      m("claude-opus-4-8-thinking-max"),
      m("claude-opus-4-8-thinking-medium"),
    ]);
    expect(result).toHaveLength(1);
    expect(result[0]!.id).toBe("claude-opus-4-8-thinking");
    expect(result[0]!.supportsEffort).toBe(true);
    expect(result[0]!.effortMap!.high).toBe("high");
    expect(result[0]!.effortMap!.xhigh).toBe("max");
  });

  test("gpt-5.5 extra-high maps to xhigh", () => {
    const result = processModels([
      m("gpt-5.5-none"),
      m("gpt-5.5-low"),
      m("gpt-5.5-medium"),
    ...
    expect(result[0]!.effortMap!.minimal).toBe("none");
    expect(result[0]!.effortMap!.xhigh).toBe("extra-high");
  });
});
...
describe("resolveModelId", () => {
  ...
  test("new Claude thinking model + effort — inserts after -thinking", () => {
    expect(resolveModelId("claude-opus-4-8-thinking", "high")).toBe(
      "claude-opus-4-8-thinking-high",
    );
    expect(resolveModelId("claude-opus-4-8-thinking", "max")).toBe(
      "claude-opus-4-8-thinking-max",
    );
  });

  test("new Claude thinking-fast + effort", () => {
    expect(resolveModelId("claude-opus-4-8-thinking-fast", "high")).toBe(
      "claude-opus-4-8-thinking-high-fast",
    );
  });

  test("extra-high effort", () => {
    expect(resolveModelId("gpt-5.5", "extra-high")).toBe("gpt-5.5-extra-high");
    expect(resolveModelId("gpt-5.5-fast", "extra-high")).toBe(
