 ...
 *
 * Google Gemini CLI and Antigravity OAuth flows were removed from Pi and are
 * deliberately not offered by this current-Pi package.
 */
...
import { createAuthStorageFacade } from "./auth-compat.ts";
import { type OAuthCredentials } from "@earendil-works/pi-ai";
import { getModels, type Api, type Model } from "@earendil-works/pi-ai/compat";
import { createLegacyOAuthProvider, type LegacyOAuthProvider } from "./oauth-adapter.ts";
import { anthropicOAuth } from "./vendor/oauth/anthropic.js";
import { openaiCodexOAuth } from "./vendor/oauth/openai-codex.js";
import {
	getGitHubCopilotBaseUrl,
	githubCopilotOAuth,
	normalizeDomain,
} from "./vendor/oauth/github-copilot.js";
import {
	Container,
	Key,
	SelectList,
...
const PROVIDER_TEMPLATES: Record<string, ProviderTemplate> = {
	anthropic: {
		...
		buildOAuth: (index) => createLegacyOAuthProvider(`Anthropic #${index}`, anthropicOAuth),
	},
	"openai-codex": {
		displayName: "ChatGPT Plus/Pro (Codex)",
		buildOAuth: (index) => createLegacyOAuthProvider(`ChatGPT Codex #${index}`, openaiCodexOAuth),
	},
	"github-copilot": {
		displayName: "GitHub Copilot",
		buildOAuth: (index) => createLegacyOAuthProvider(`GitHub Copilot #${index}`, githubCopilotOAuth),
		buildModifyModels(providerName) {
...
function normalizeEntries(entries: SubEntry[]): SubEntry[] {
	...
	for (const entry of entries) {
		if (!PROVIDER_TEMPLATES[entry.provider]) continue;
...
async function showSubscriptionActions(
	...
	if (action === "logout") {
		if (!(await getAuthStorage(ctx).logout(name))) {
			ctx.ui.notify(
				`${subDisplayName(entry)} uses non-stored credentials; remove them from their source instead.`,
				"warning",
			);
			return;
		}
...
async function handleSubsLogout(ctx: ExtensionCommandContext): Promise<void> {
	...
	if (!(await getAuthStorage(ctx).logout(subProviderName(entry)))) {
		ctx.ui.notify(
			`${subDisplayName(entry)} uses non-stored credentials; remove them from their source instead.`,
			"warning",
		);
