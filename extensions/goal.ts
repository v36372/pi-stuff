// Vendored from Armin Ronacher's agent-stuff repo.
// Source: https://github.com/mitsuhiko/agent-stuff/blob/main/extensions/goal.ts
// To update from the dotfiles repo root:
//   { printf '%s\n' \
//       "// Vendored from Armin Ronacher's agent-stuff repo." \
//       "// Source: https://github.com/mitsuhiko/agent-stuff/blob/main/extensions/goal.ts" \
//       "// To update from the dotfiles repo root: see this header." \
//       ""; \
//     curl -fsSL https://raw.githubusercontent.com/mitsuhiko/agent-stuff/main/extensions/goal.ts; \
//   } > home/.pi/agent/extensions/goal.ts
...
export default function goalExtension(pi: ExtensionAPI) {
	...
	pi.registerCommand("goal", {
		...
		handler: async (args, ctx) => {
			...
			if (goal) {
				if (!ctx.hasUI) {
					showGoalMessage("A goal already exists. Run /goal clear first, or use interactive mode to confirm replacement.");
					return;
				}
