...
function parseFilePathArgument(args: string): string | null {
...
function toRepoRelativePath(repoRoot: string, baseDir: string, filePath: string): string | null {
...
function resolveRequestedPaths(repoRoot: string, cwd: string, filePath: string): string[] {
  return [...new Set([
    toRepoRelativePath(repoRoot, cwd, filePath),
    toRepoRelativePath(repoRoot, repoRoot, filePath),
  ].filter((candidate): candidate is string => candidate != null))];
}

function matchesRequestedPath(file: ReviewFile, requestedPath: string): boolean {
  return [
    file.path,
    file.gitDiff?.oldPath,
    file.gitDiff?.newPath,
    file.lastCommit?.oldPath,
    file.lastCommit?.newPath,
  ].some((candidate) => candidate === requestedPath);
}

function notifyReviewFailure(ctx: ExtensionCommandContext, error: unknown): void {
...
export default function (pi: ExtensionAPI) {
  ...
  async function openReviewWindow(ctx: ExtensionCommandContext, repoRoot: string, files: ReviewFile[]): Promise<void> {
  ...
  async function reviewRepository(ctx: ExtensionCommandContext): Promise<void> {
    ...
    try {
      ...
      await openReviewWindow(ctx, repoRoot, files);
    } catch (error) {
      notifyReviewFailure(ctx, error);
    }
  }

  async function reviewSingleFile(args: string, ctx: ExtensionCommandContext): Promise<void> {
    if (activeWindow != null) {
      ctx.ui.notify("A review window is already open.", "warning");
      return;
    }

    const filePathArg = parseFilePathArgument(args);
    if (filePathArg == null) {
      ctx.ui.notify("Usage: /plan-review <file-path>", "error");
      return;
    }

    try {
      const { repoRoot, files } = await getReviewWindowData(pi, ctx.cwd);
      const requestedPaths = resolveRequestedPaths(repoRoot, ctx.cwd, filePathArg);
      if (requestedPaths.length === 0) {
        ctx.ui.notify("The file path must resolve to a file inside the current git repository.", "error");
        return;
      }

      const matchingFiles = files.filter((file) => requestedPaths.some((requestedPath) => matchesRequestedPath(file, requestedPath)));
      if (matchingFiles.length === 0) {
        ctx.ui.notify(`No reviewable file found for ${requestedPaths[0]}.`, "info");
        return;
      }

      await openReviewWindow(ctx, repoRoot, matchingFiles);
    } catch (error) {
      notifyReviewFailure(ctx, error);
    }
  }
  ...
  pi.registerCommand("plan-review", {
    description: "Open the native review window for a single file. Usage: /plan-review <file-path>",
    handler: async (args, ctx) => {
      await reviewSingleFile(args, ctx);
    },
  });
