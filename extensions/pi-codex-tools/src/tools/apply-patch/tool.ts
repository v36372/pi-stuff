...
interface ApplyPatchToolOptions {
	/** Static dir or live resolver so config edits apply without reload. */
	customRustBinariesDir?: string | (() => string | undefined) | undefined;
	promptSnippet?: boolean | undefined;
	showDiffWhenCollapsed?: boolean | undefined;
}

function resolveCustomRustBinariesDir(options: ApplyPatchToolOptions): string | undefined {
	const value = options.customRustBinariesDir;
			...
			try {
				result = await withTouchedFileMutationQueues(ctx.cwd, typedParams.patchText, () =>
					executePatchWithRust({
						...
						signal,
						customRustBinariesDir: resolveCustomRustBinariesDir(options),
					}),
				);
			} catch (error) {
